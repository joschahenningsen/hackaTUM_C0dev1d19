...
class ChangeDetails {
    ...
    List<Change> getChanges() {
        ...
        if (Float.compare(FiO2_Old, Float.parseFloat(FiO2)) != 0) {
            ret.add(new Change(device_id, "FiO₂", FiO2_Old + "%", Float.parseFloat(FiO2) + "%"));
        }
        if (Float.compare(IE_Old, IE) != 0) {
            ret.add(new Change(device_id, "IE", IE_Old + "", IE + ""));
        }
        if (Float.compare(MVe_Old, Float.parseFloat(MVe)) != 0) {
            ret.add(new Change(device_id, "MVe", MVe_Old + " mL", Float.parseFloat(MVe) + " mL"));
        }
        ...
        if (Float.compare(humidity_Old, Float.parseFloat(humidity)) != 0) {
            ret.add(new Change(device_id, "humidity", humidity_Old + "%", Float.parseFloat(humidity) + "%"));
        }
        if (Integer.parseInt(pressure_max) != pressure_max_Old) {
            ret.add(new Change(device_id, "Pmax", pressure_max_Old + " cmH₂O", Integer.parseInt(pressure_max) + " cmH₂O"));
        }
        if (VT_Old != VT) {

        ...
        while not self._stopevent.is_set():
            ...
            if msg[0] == "start":
                ...
                for key,value in req.items():
                    ...
                    print(temp['IE'])
                    cursor.execute("INSERT INTO screenshots (fio2,ie,mve,peep,rr,vt, humidity, pressure_max, id,vent) VALUES(%s,%s, %s, %s, %s, %s, %s, %s, $token$%s$token$,$token$%s$token$)" % (temp['FiO2'],temp['IE'],temp['MVe'],temp['PEEP'],temp['RR'],temp['VT'], temp['humidity'], temp['pressure_max'],msg[1],key))
                    conndb.commit()
                cursor.close()
            elif msg[0] == "resume":
                print(msg[1])
                ...
                for row in rows:
                    ...
                    dict2.append({'device_id':req[row[8]]['device_id'],'FiO2_Old':row[0],'FiO2':temp['FiO2'], 'IE_Old':row[1],'IE':temp['IE'],'MVe_Old':row[2],'MVe':temp['MVe'],'PEEP_Old':row[3],'PEEP':temp['PEEP'],'RR_Old':row[4],'RR':temp['RR'],'VT_Old':row[5],'VT':temp['VT'],'humidity_Old':row[6],'humidity':temp['humidity'],'pressure_max_Old':row[7],'pressure_max':temp['pressure_max']})

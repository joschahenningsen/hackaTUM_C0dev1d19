        ...
        while not self._stopevent.is_set():
            ...
            if msg[0] == "start":
                ...
                for key,value in req.items():
                    ...
                    print(temp['IE'])
                    cursor.execute("INSERT INTO screenshots (fio2,ie,mve,peep,rr,vt, humidity, pressure_max, id,vent) VALUES(%s,%e, %s, %e, %d, %d, %s, %s, $token$%s$token$,$token$%s$token$)" % (temp['FiO2'],temp['IE'],temp['MVe'],temp['PEEP'],temp['RR'],temp['VT'], temp['humidity'], temp['pressure_max'],msg[1],key))
                    conndb.commit()
                cursor.close()
            elif msg[0] == "resume":
                print(msg[1])
